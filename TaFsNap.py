...
if "error_message" in response.text:
                print('' + user + ' --> There is no account associated with it | ! صاحب الحساب موثق رقمه')
                with open('الارقام_الموثقه', 'a') as x:
                    x.write(user + '\n')
elif "phone_number" in response.text:   
                print('' + user + ' --> There is an account associated with it | ! غير موثق')
                with open('الارقام_الغير_موثقه.txt', 'a') as x:
                    x.write(user + '\n')
